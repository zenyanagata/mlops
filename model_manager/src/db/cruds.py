...
def select_experiment_by_project_id(db: Session, project_id: str) -> List[schemas.Experiment]:
    return db.query(models.Experiment, models.Model).filter(models.Model.project_id == project_id).filter(models.Experiment.model_id == models.Model.model_id).all()
