...
def add_project(project: schemas.ProjectCreate, db: Session = Depends(get_db)):
    return cruds.add_project(db=db, project_name=project.project_name, description=project.description, commit=True)
...
def update_artifact_file_paths(
    ...
    return cruds.update_experiment_artifact_file_paths(
        ...
        artifact_file_path=artifact_file_paths.artifact_file_paths
    )
